...
public:
  ...
  {
    // convert gnss to lidar coord
    nav_msgs::Odometry gnss_msg = gnssConverter(gpsMsg);
    gpsQueue.push_back(gnss_msg);
  }
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void printTransfromTobeMapped(const std::string& print_info)
  {
    std::cout << print_info << std::endl;
    std::cout << "xyz: " << transformTobeMapped[3] << " " << transformTobeMapped[4] << " " << transformTobeMapped[5]
              << std::endl;
    std::cout << "rpy: " << transformTobeMapped[0] << " " << transformTobeMapped[1] << " " << transformTobeMapped[2]
              << std::endl;
  }

  bool detectGnss(const double lidar_time)
  {
    if (gpsQueue.size() < 2)
    {
      return false;
    }
    bool gnss_detected = false;
    Pose matched_gnss;
    auto iter = gpsQueue.begin() + 1;
    for (; iter != gpsQueue.end(); ++iter)
    {
      const auto& last_gnss_time = (iter - 1)->header.stamp.toSec();
      const auto& curr_gnss_time = iter->header.stamp.toSec();
      if (last_gnss_time <= lidar_time && curr_gnss_time >= lidar_time)
      {
        auto time_diff_front = lidar_time - last_gnss_time;
        auto time_diff_back = curr_gnss_time - lidar_time;
        if (time_diff_front <= 0.1 && time_diff_back <= 0.1)
        {
          double ratio = time_diff_back / (time_diff_front + time_diff_back);
          const auto& last_gnss_p = (iter - 1)->pose.pose.position;
          const auto& last_gnss_q = (iter - 1)->pose.pose.orientation;
          const auto& curr_gnss_p = iter->pose.pose.position;
          const auto& curr_gnss_q = iter->pose.pose.orientation;
          Eigen::Vector3d p_front = Eigen::Vector3d(last_gnss_p.x, last_gnss_p.y, last_gnss_p.z);
          Eigen::Quaterniond q_front = Eigen::Quaterniond(last_gnss_q.w, last_gnss_q.x, last_gnss_q.y, last_gnss_q.z);
          Eigen::Vector3d p_back = Eigen::Vector3d(curr_gnss_p.x, curr_gnss_p.y, curr_gnss_p.z);
          Eigen::Quaterniond q_back = Eigen::Quaterniond(curr_gnss_q.w, curr_gnss_q.x, curr_gnss_q.y, curr_gnss_q.z);

          matched_gnss.time_stamp = iter->header.stamp.toSec();
          matched_gnss.position = p_back - ratio * (p_back - p_front);
          matched_gnss.q = q_back.slerp(ratio, q_front);
          matched_gnss.q.normalize();
          gnss_detected = true;
        }
        break;
      }
    ...
    if (gnss_detected)
    {
      cur_gps_ = matched_gnss;
      if (iter > gpsQueue.begin() + 1)
      {
        gpsQueue.erase(gpsQueue.begin(), iter - 2);
      }
    }
  ...
  void updateInitialGuess()
  {
    ...
    if (cloudKeyPoses3D->points.empty())
    {
      ...
      ROS_INFO("use imu init first rpy!!!");
      ...
      if (detectGnss(timeLaserInfoCur))
      {
        Eigen::Vector3d euler_angle = getEulerAngle(cur_gps_.q);
        transformTobeMapped[0] = euler_angle(0);
        transformTobeMapped[1] = euler_angle(1);
        transformTobeMapped[2] = euler_angle(2);
        transformTobeMapped[3] = cur_gps_.position.x();
        transformTobeMapped[4] = cur_gps_.position.y();
        transformTobeMapped[5] = cur_gps_.position.z();

        ROS_INFO("use gnss init first position!!!");
        std::cout << "first frame time: " << std::setprecision(10) << timeLaserInfoCur << std::endl;
        std::cout << "first frame xyz: " << transformTobeMapped[3] << " " << transformTobeMapped[4] << " "
                  << transformTobeMapped[5] << std::endl;
        std::cout << "first frame rpy: " << transformTobeMapped[0] << " " << transformTobeMapped[1] << " "
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void addGPSFactor()
  {
    ...
    if (detectGnss(timeLaserInfoCur))
    {
      ...
      float gps_x = cur_gps_.position.x();
      float gps_y = cur_gps_.position.y();
      float gps_z = cur_gps_.position.z();
      ...
      if (pointDistance(curGPSPoint, lastGPSPoint) > 5.0)
      {
        ...
        aLoopIsClosed = true;
        ROS_INFO("add gnss factor!!!");
        std::cout << "gnss pose: " << gps_x << " " << gps_y << " " << gps_z << std::endl;
        std::cout << "curr pose: " << transformTobeMapped[3] << " " << transformTobeMapped[4] << " "
                  << transformTobeMapped[5] << std::endl;
      }
    }
  }

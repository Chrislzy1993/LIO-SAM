...
public:
  ...
  // converter into lidar coordinate
  nav_msgs::Odometry gnssConverter(const nav_msgs::Odometry::ConstPtr& gnss_msg)
  {
    Eigen::Vector3d gnss_p(gnss_msg->pose.pose.position.x, gnss_msg->pose.pose.position.y,
                           gnss_msg->pose.pose.position.z);
    Eigen::Quaterniond gnss_q(gnss_msg->pose.pose.orientation.w, gnss_msg->pose.pose.orientation.x,
                              gnss_msg->pose.pose.orientation.y, gnss_msg->pose.pose.orientation.z);
    Eigen::Matrix3d lidar2imu_r = extRot.transpose();
    Eigen::Vector3d lidar2imu_t = -lidar2imu_r * extTrans;
    gnss_p = gnss_q * lidar2imu_t + gnss_p;
    gnss_q = Eigen::Quaterniond(gnss_q * lidar2imu_r);
    nav_msgs::Odometry msg = *gnss_msg;
    msg.pose.pose.position.x = gnss_p.x();
    msg.pose.pose.position.y = gnss_p.y();
    msg.pose.pose.position.z = gnss_p.z();
    msg.pose.pose.orientation.x = gnss_q.x();
    msg.pose.pose.orientation.y = gnss_q.y();
    msg.pose.pose.orientation.z = gnss_q.z();
    msg.pose.pose.orientation.w = gnss_q.w();
    return msg;
  }
...
Eigen::Vector3d getEulerAngle(const Eigen::Quaterniond& rot_q)
{
  tf::Quaternion rot_q_tf;
  tf::quaternionEigenToTF(rot_q, rot_q_tf);
  Eigen::Vector3d rot_eular = Eigen::Vector3d::Zero();
  tf::Matrix3x3(rot_q_tf).getRPY(rot_eular(0), rot_eular(1), rot_eular(2));
  return rot_eular;
}
